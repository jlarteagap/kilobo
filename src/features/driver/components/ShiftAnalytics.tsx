...
export function ShiftAnalytics() {
  ...
  if (isLoading) {
    return (
      <div className="space-y-6 animate-pulse">
        ...
        <div className="h-48 bg-[#F2F9E3]/40 dark:bg-neutral-900 rounded-2xl" />
  ...
  if (analytics.summary.shiftCount === 0) {
    return (
      <div className="p-12 text-center border border-dashed border-[rgba(0,0,0,0.12)] dark:border-neutral-800 rounded-2xl">
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* ── Breakdown bruto ── */}
      <div className="rounded-[22px] bg-white dark:bg-neutral-900/50 p-6"
        style={{ boxShadow: '0 2px 16px rgba(0,0,0,0.05)' }}
      >
        <h3 className="text-xs font-bold text-[#6E6E73] uppercase tracking-widest mb-5">Desglose bruto</h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
          <div className="p-3 rounded-xl bg-[#F2F9E3]/40 dark:bg-neutral-900/40">
            <p className="text-[9px] uppercase tracking-widest text-[#6E6E73] font-bold">Bruto total</p>
            <p className="text-lg font-bold text-foreground dark:text-neutral-100 tabular-nums mt-1">{formatBs(s.grossEarnings)}</p>
          </div>
          ...
          <div className="p-3 rounded-xl bg-[#FAEDE9] dark:bg-rose-950/10">
            ...
            return (
              <div key={app.app} className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold tabular-nums text-foreground dark:text-neutral-100">
                ...
                {/* Barra comparativa */}
                <div className="h-1.5 rounded-full bg-[rgba(0,0,0,0.06)] dark:bg-neutral-800 overflow-hidden">
                  <div
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={analytics.dailyTrend} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" className="text-[rgba(0,0,0,0.08)] dark:text-neutral-800" stroke="currentColor" />
                <XAxis
                  dataKey="date"
...
function SummaryCard({
  ...
  const colorMap = {
    emerald: 'bg-[#F2F9E3] dark:bg-emerald-950/20 text-[#4F6A35] dark:text-emerald-400 border-[#C8D9A9] dark:border-emerald-900/50',
