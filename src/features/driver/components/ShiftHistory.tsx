  ...
  if (shifts.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-neutral-200 dark:border-neutral-800 p-8 text-center">
        <div className="size-10 rounded-xl bg-neutral-50 dark:bg-neutral-900 mx-auto flex items-center justify-center text-neutral-300 mb-3">
          <History className="size-5" />
        </div>
        <p className="text-xs text-neutral-400 italic">Aún no hay turnos registrados</p>
      </div>
    )
  }
        ...
        {shifts.slice(0, 50).map((shift, index) => {
          ...
          return (
            <div
              ...
              className="group relative flex items-center gap-3 p-3 sm:p-4 rounded-xl bg-white dark:bg-neutral-900/40 border border-neutral-100 dark:border-neutral-900 hover:border-neutral-200 dark:hover:border-neutral-800 transition-all duration-200 hover:shadow-sm cursor-pointer"
            >
              {/* Fecha */}
              <div className="min-w-[36px] text-center shrink-0">
                <p className="text-[16px] font-bold text-neutral-900 dark:text-neutral-100 leading-none tabular-nums">
                  {isValidDate ? date.getDate() : '—'}
                </p>
                <p className="text-[8px] uppercase tracking-wider text-neutral-400 font-medium mt-0.5">
                  {isValidDate ? date.toLocaleDateString('es-BO', { month: 'short' }).replace('.', '') : ''}
                </p>
              </div>

              {/* Horas */}
              <div className="hidden sm:flex items-center gap-1.5 text-[11px] text-neutral-400 min-w-[60px]">
              ...
              <div className="text-right ml-auto">
                <p className="text-sm font-bold text-emerald-600 tabular-nums leading-none">
                  {liquid.toFixed(0)}
                </p>
                {pending > 0 && (
                  <p className="text-[9px] text-neutral-400 tabular-nums mt-0.5">
              ...
              {/* Tendencia */}
              {trend != null && trend !== 0 && (
                <div className={`hidden sm:flex items-center gap-0.5 text-[10px] font-bold shrink-0 ${trend > 0 ? 'text-emerald-500' : 'text-rose-400'}`}>
              ...
              <div className="absolute -top-1.5 -right-1.5 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-all duration-200">
                {onEdit && (
                  <button
                    ...
                    className="size-7 rounded-full bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 flex items-center justify-center text-neutral-300 hover:text-blue-500 hover:border-blue-200 dark:hover:border-blue-800 shadow-sm"
                    title="Editar turno"
                  >
                ...
                <button
                  ...
                  className="size-7 rounded-full bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 flex items-center justify-center text-neutral-300 hover:text-red-500 hover:border-red-200 dark:hover:border-red-800 shadow-sm"
